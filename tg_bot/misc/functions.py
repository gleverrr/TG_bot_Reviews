...
def create_folder(order_id):
    ...
    return {"base": base_path, "photos": photos_path, "video": videos_path}


async def put_photo_files(
    state: FSMContext, message: types.Message, bot: Bot, BOT_TOKEN, number
):
    ...
    download_url = (
        f"https://api.telegram.org/file/bot{BOT_TOKEN}/{photo_file.file_path}"
    )
...
async def save_video_file(
    state: FSMContext, message: types.Message, bot: Bot, BOT_TOKEN
):
    data = await state.get_data()
    order_id = data.get("order_id")
    paths = create_folder(order_id)
    ...
    download_url = (
        f"https://api.telegram.org/file/bot{BOT_TOKEN}/{video_file.file_path}"
    )
    file_response = requests.get(download_url)
    if file_response.status_code == 200:
